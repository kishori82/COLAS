...
void algorithm_SODAW_WRITE_PUT( zmsg_t *msg, void *worker, char *object_name) {
    char tag_str[100];
...
void algorithm_SODAW_WRITE_GET_OR_READ_GET_TAG( zmsg_t *msg, void *worker, char *object_name) {
     char buf[100];
     TAG tag;
...
void algorithm_SODAW( zmsg_t *msg, void *worker, char *object_name) {
     ...
     printf("algorithm SODAW tag %s\n", buf);
      if( strcmp(buf, WRITE_GET)==0)  {
           printf("\t-----------------\n");
           printf("\tWRITE_GET_TAG\n");
           algorithm_SODAW_WRITE_GET_OR_READ_GET_TAG(msg, worker, object_name);
      }

      if( strcmp(buf, WRITE_PUT)==0)  {
           printf("\t-----------------\n");
           printf("\tREAD VALUE\n");
           algorithm_SODAW_WRITE_PUT(msg, worker, object_name);
      }

  
      if( strcmp(buf, READ_GET)==0)  {
           printf("\tWRITE_VALUE\n");
           algorithm_SODAW_WRITE_GET_OR_READ_GET_TAG(msg, worker, object_name);
      }

      if( strcmp(buf, READ_VALUE)==0)  {
           printf("\t-----------------\n");
           printf("\tREAD VALUE\n");
